...
x 축은 모형 단계의 단일 switch 이며 `find_maturity()` 가 한 점 ($k^*$)
...
| `CohortRegime` 객체 | `detect_regime()` 결과를 직접 전달      |
...
1. 1차 pass: 필터링 없는 raw triangle 으로 `find_maturity()` 호출 →
...
- `vignette("backtest")` — `recent`, `regime_break` 가 결과에 미치는
  영향을 진단하는 도구.
- `vignette("regime-detection")` — `detect_regime()` 참고.
