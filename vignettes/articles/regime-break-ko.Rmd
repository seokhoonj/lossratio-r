---
title: "Regime break 기반 hybrid 필터링"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Regime break 기반 hybrid 필터링}
...
x 축은 모형 단계의 단일 switch 이며 `find_ata_maturity()` 가 한 점 ($k^*$)
...
| `CohortRegime` 객체 | `detect_cohort_regime()` 결과를 직접 전달      |
...
1. 1차 pass: 필터링 없는 raw triangle 으로 `find_ata_maturity()` 호출 →
...
`detect_cohort_regime()` 은 단일 그룹 triangle 을 전제한다. 여러
...
- `vignette("loss-ratio-methods")` — `fit_lr()` 과 `"sa"`, `"ed"`,
  `"cl"` 방법.
- `vignette("backtest")` — `recent`, `regime_break` 가 결과에 미치는
  영향을 진단하는 도구.
- `vignette("regime-detection")` — `detect_cohort_regime()` 참고.
- `?fit_lr`, `?fit_ata`, `?fit_ed`, `?detect_cohort_regime`.
