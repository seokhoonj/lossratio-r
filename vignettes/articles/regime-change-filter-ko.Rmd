...
`loss_regime` (와 premium-side 짝 `prem_regime`) 인자가 이 직관을
그대로 구현한다.
...
`prem_regime` (premium-side 필터; default 는 `loss_regime` 와 동일)
을 받는다. `fit_loss()` / `fit_prem()` 은 단일 `regime` 인자.
`backtest()` 는 `fit_lr` 와 동일하게 `loss_regime` / `prem_regime`
