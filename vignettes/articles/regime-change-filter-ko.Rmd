...
`exposure_regime` (exposure-side 필터; default 는 `loss_regime` 와 동일)
을 받는다. `fit_loss()` / `fit_exposure()` 은 단일 `regime` 인자.
`backtest()` 는 `fit_ratio` 와 동일하게 `loss_regime` / `exposure_regime`
...
단순 모드 (`fit_ratio(method ∈ {"ed","cl"})`) 에서는 change 이전 코호트를
...
![surgery triangle 에서 네 가지 필터 설정이 사용하는 셀. 파랑 = 적합에 사용,
...
내부 `experience` 데이터셋의 surgery 보장은 24.04 에 합성 regime change 가
...
향후 `loss_regime = list(surgery = regime_at(...), cancer = regime_at(...))`
...
- `?fit_ratio`, `?fit_ata`, `?fit_ed`, `?detect_regime`, `?regime_at`.
