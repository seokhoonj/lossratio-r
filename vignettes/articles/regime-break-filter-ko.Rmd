---
title: "Regime break filter: 탐지된 regime 을 fit_* 의 필터로"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Regime break filter: 탐지된 regime 을 fit_* 의 필터로}
...
> 영어 원본 보기: [Hybrid filtering via regime break](../regime-break.html)
...
수도 있다. `Regime` 객체가 다중 breakpoint 를 담고 있을 때는 가장 최신
...
break 날짜를 직접 지정하려면 `regime_at(breakpoint = "2024-07-01")` 로
...
성숙점 $k^*$ 자체는 break 컷에 의해 영향을 받지 않도록 **2-pass 검출** 로
추정한다.

1. 1차 pass: 필터링 없는 raw triangle 으로 `detect_maturity()` 호출 →
   $k^*$ 추정. break 이전·이후 코호트 모두 ATA 패턴에 기여하므로 $k^*$
   는 안정적이다.
...
post-break window 가 너무 짧으면 (`n_post` 가 작으면) ED 강도 $g_k$
...
공유한다. break 이전 코호트의 ultimate 추정도 post-break 데이터로
