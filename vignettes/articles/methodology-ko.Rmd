...
`detect_maturity()` 의 $k^*$ 이후, observed dev range 의 *끝* 에서 *진짜
ultimate* 까지는 *외삽* 이 필요.

Sherman 1984 의 *log-linear (exponential form) tail* 은 이미 패키지에
있다 — `fit_cl(tail = TRUE)` 가 selected factor 위에서
`log(f_k - 1) ~ k` 적합 + 100 future link 외삽. 한계: *exponential form
만* / *fit_cl 내부 캡슐화* / *진단·SE surface 없음* / *horizon 하드코드* /
*silent fallback*.
...
기존 embedded Sherman exp 대비 *세 가지 확장*:

-   추가 Sherman curve (inverse_power, Weibull) + Clark 2003 의
    parametric growth curve family (loglogistic / Weibull MLE).
-   *Standalone 진입* — *어떤 dev-decay series* 든 받음 (f, g,
    ratio path 등). fit_cl 한정 아님.
-   *진단 surface* — 계수·잔차·`fit_dev_range` 민감도·tail factor SE.
