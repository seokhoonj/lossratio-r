...
기본 추정 대상은 누적 손해율 (`target = "ratio"`) 이며, 손해 측 method 는
단계 적응형(stage-adaptive, SA, `loss_method = "sa"`) 이다. 반환되는 객체는
`"Backtest"` 리스트이며, 주요 슬롯은 다음과 같다.

- `ae_err` — 셀 단위 `data.table` (cohort, dev, actual, pred, ae_err, cal_idx).
...
대각선을 가로지르는 단조로운 표류 (위 surgery 예시처럼 `25, ..., 30` 으로
...
기본값인 `target = "ratio"` (`loss_method = "sa"`) 은 손해율 관점의 진단을
...
- `vignette("projection")` — `fit_ratio()` 및 `"sa"`, `"ed"`, `"cl"`
  방법.
