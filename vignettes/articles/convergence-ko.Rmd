...
`backtest()` 를 돌려 각 시점의 예측 손해율 경로 `lr` 을 만들고, 그 경로
...
holdout_max` 인 $k$ 만 finite `lr[i]` 값을 받고, 나머지는 `NA` 로
마스킹.
...
3. `detect_convergence()` 로 $k^{**} \ge k^*$ 검출. 예측 손해율은
   `fit_lr()$summary` 에서 읽고 위의 reserving 주의사항을 적용.
...
- **식별 가능성**: `conv_k` 는 `dev_max - mat_k >= window` (또는 tail
  ...
  method 가 `NA` 반환.
- **모형 조건부**: 예측 LR 은 `fit_lr()` 로 산출. `fit_lr()` 이
  ...
  `fit_lr` 설정을 같이 확인할 것. `...` 으로 `loss_method =`,
