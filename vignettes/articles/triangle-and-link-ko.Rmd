---
title: "Triangle 와 Link: 데이터 구조와 인자 진단"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Triangle 와 Link: 데이터 구조와 인자 진단}
...
> 영어 원본 보기: [Triangle and Link: data structures and factor diagnostics](../triangle-and-link.html)

chain ladder 또는 손해율 모형을 적합하기 전에 기반이 되는 triangle 과 그로부터
파생된 단계 연결 테이블(link table) 을 살펴보는 것이 효율적이다. 이 문서는
`Triangle` 과 `Link` 데이터 구조 및 진단 플롯을 다룬다. 성숙점(maturity point)
탐지는 `vignette("maturity")` 를 참고한다.
...
- `vignette("maturity")` — link table 기반 성숙점 탐지.
