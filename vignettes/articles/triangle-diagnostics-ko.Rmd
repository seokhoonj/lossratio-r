...
것이 효율적이다. 이 문서는 코호트 발전 양상, **ATA 인자**(age-to-age factor) 의
안정성, 성숙점(maturity point) 탐지를 이해하기 위한 `lossratio` 의 진단 도구를
다룬다.
...
`Link` 객체 (단일 변수 모드) 의 `summary()` 메소드는 성숙점 탐지를 구동하는 링크별 통계를 계산한다.
...
`find_maturity()` 는 단일 변수 `Link` 객체의 `summary()` 결과를 입력으로 받는다 —
...
`find_maturity()` 는 `maturity_args` 가 주어진 경우 `fit_ata()` 와
...
5. `build_link()` + `plot(ata, type = "cv")` — 링크 안정성 확인.
6. `find_maturity()` — 그룹별로 합리적 성숙점이 잡히는지 확인.
...
- `vignette("regime-detection")` — `detect_regime()` 심화.
