...
|----------------------|---------------|-----------------------------|--------------------------------------------|
| `as_triangle()`   | `Triangle`    | cohort × dev (2D)           | SA, ED, CL projection                  |
| `as_calendar()`   | `Calendar`    | calendar period (1D)        | Calendar-year trend, diagonal effect       |
| `as_total()`      | `Total`       | portfolio total (per group) | High-level loss-ratio comparison           |
...
- `as_link()` — development factors (ATA / ED via `loss` + optional `exposure`)
- `fit_cl()`, `fit_ratio()` — projection
...
sequential index (1, 2, 3, ...) within group — time-series
convention. It is **not** a development period (`cym - uym`); for
that you want the `Triangle` `dev` axis.
...
Granularity (`"month"` / `"quarter"` / `"semi-annual"` / `"annual"`)
is derived on demand from the raw column name via
