...
| `"prem"` | `fit_prem()`    | `prem_method` | `prem_proj`     |
...
classical chain ladder. The `prem_method` argument selects the
premium projection strategy when `target = "prem"`.
...
`target = "loss"` or `target = "prem"` to score the corresponding
