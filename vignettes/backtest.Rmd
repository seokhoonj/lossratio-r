...
premium-weighted pooled A/E ratio minus 1. Comparing the three
...
|--------------|-------------------|-------------------|-------------------|
| `"ratio"`    | `fit_ratio()`     | `loss_method`     | `ratio_proj`      |
...
unconditional safe baseline -- no maturity or regime detection
needed; `"cl"` is the classical chain ladder, which lets the
cohort's own cum_loss anchor cohort-level drift; `"sa"`
(stage-adaptive) blends ED before the maturity point with CL
afterwards. The `premium_method` argument selects the premium
...
- `vignette("projection")` — `fit_cl()` reference.
- `vignette("projection")` — `fit_ratio()` and the `"sa"`,
  `"ed"`, `"cl"` methods.
