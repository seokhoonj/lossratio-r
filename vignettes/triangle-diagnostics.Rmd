...
The `summary()` method on an `ATA` object computes per-link statistics
that drive maturity detection:
...
`find_ata_maturity()` operates on a summary of an `ATA` object — first
build the descriptive / WLS summary via `summary()`, then probe it for
...
The `summary()` method on an `ED` object is the ED-side analogue of
`summary()` on an `ATA` object, computing per-link statistics for the
...
5. `build_ata()` + `plot(ata, type = "cv")` — link stability.
