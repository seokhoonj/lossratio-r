...
(loss-side filter) and `prem_regime` (premium-side filter; defaults
to `loss_regime`). `fit_loss()` / `fit_prem()` take a single
`regime` argument. `backtest()` mirrors `fit_lr` with `loss_regime` /
`prem_regime`. All four accept the same input types:
