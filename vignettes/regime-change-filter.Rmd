---
title: "Regime change filter: hybrid cohort + diagonal cut for fit_*"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Regime change filter: hybrid cohort + diagonal cut for fit_*}
...
`loss_regime` (and its exposure-side sibling `exposure_regime`) implements
that split.
...
- `treatment = "segment_wise"`: preserve every change. Each segment
  ...
  e.g., multi-regime + long-tail data where the latest regime hasn't
  yet observed late-dev development.
...
(loss-side filter) and `exposure_regime` (exposure-side filter; defaults
to `loss_regime`). `fit_loss()` / `fit_exposure()` take a single
`regime` argument. `backtest()` mirrors `fit_ratio` with `loss_regime` /
`exposure_regime`. All four accept the same input types:
...
In simple modes (`fit_ratio(method ∈ {"ed","cl"})`) the same argument acts
...
with `regime_at(change = "2024-07-01")` to construct a `Regime`
...
first on the raw triangle (so noisy post-change windows do not destabilise
...
- **drift (cal30 − cal25)**: A/E Error aggregated by calendar diagonal,
  ...
  window — the signature of a regime change that the static model has
  not absorbed.
...
A future extension may accept `loss_regime = list(surgery = regime_at(...),
cancer = regime_at(...))`. Today only `NULL` / `Regime` / `"auto"` /
