...
`find_maturity()` answers the question *"from which development
...
`find_convergence()` detects the **convergence point** $k^{**}$ —
...
(maturity point, from `find_maturity()`): $k^*$ marks where link
...
| $k^*$ | maturity point (from `find_maturity()`); lower bound on candidate $v$ |
| $k^{**}$ | convergence point — the value `find_convergence()` returns |
...
- `k_star` — the maturity point used as the lower bound (computed
  internally via `find_maturity()` on a clr-based ATA, or supplied
  by the caller).
...
`find_convergence()` refits the model at each candidate valuation
...
sapply(
  c(0.25, 0.5, 0.75, 1.0),
  function(cc) find_convergence(tri, c = cc)$k_conv
)
```
...
| `find_maturity()` ($k^*$) | When are link factors reproducible? | a dev value | development period |
| `find_convergence()` ($k^{**}$) | When does the LR estimate stop revising? | a dev value | development period |
...
2. For each homogeneous group, compute $k^*$ via
   `find_maturity()`.
3. Run `find_convergence()` to obtain $k^{**} \ge k^*$. The reported
   $\hat{clr}^{\mathrm{stable}}$ is the LR averaged over
   $k \ge k^{**}$ (or projected via `fit_lr()`).
...
`find_convergence()` is a thin layer over repeated `backtest()` calls
...
- `?find_convergence`, `?find_maturity`, `?backtest`,
  `?detect_regime`.
