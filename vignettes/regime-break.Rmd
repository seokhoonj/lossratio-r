---
title: "Hybrid filtering via regime break"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Hybrid filtering via regime break}
...
`find_ata_maturity()`. Regime breaks are exogenous events — there can
...
| `CohortRegime` object       | output of `detect_cohort_regime()` passed in    |
...
- **overall mean**: cell-level mean AEG across all held-out cells —
...
`detect_cohort_regime()` assumes a single-group triangle. For a
portfolio with multiple `cv_nm` groups, call it per group:
...
A future extension may accept `regime_break = list(SUR = "2024-04-01",
CAN = "2023-12-01")`. Today only scalar / vector / `CohortRegime` are
supported.
...
- `vignette("backtest")` — diagnosing the impact of `recent` and
  `regime_break`.
- `vignette("regime-detection")` — `detect_cohort_regime()` reference.
