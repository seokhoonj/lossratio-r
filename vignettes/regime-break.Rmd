...
`find_maturity()`. Regime breaks are exogenous events — there can
...
| `CohortRegime` object       | output of `detect_regime()` passed in    |
...
- `vignette("backtest")` — diagnosing the impact of `recent` and
  `regime_break`.
- `vignette("regime-detection")` — `detect_regime()` reference.
