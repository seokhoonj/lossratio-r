...
- Standard reserving practice. Equivalent to `fit_cl(tri, target = "loss")`
  for the loss projection, but `fit_lr()` additionally projects
  exposure forward via CL on `premium` and computes the loss-ratio
...
`fit_lr()` reports analytical standard errors for `L/P`. Two variants:
...
projection switches from ED to CL. By default `fit_lr()` /
