...
column. Unlike `fit_ratio()` (which projects loss / exposure jointly to
...
`loss` selects the cumulative column to project — typically
`"loss"` (cumulative loss) for reserving, or `"exposure"` (cumulative
...
- `vignette("projection")` — when to use `fit_ratio()` instead.
