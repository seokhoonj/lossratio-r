...
additive). ELR-based reserve methods: `fit_bf` (Bornhuetter-Ferguson
with external prior), `fit_cc` (Cape Cod with data-pooled ELR estimate).
Role dispatchers: `fit_loss` (loss-side sa/ed/cl), `fit_exposure`
(exposure-side ed/cl). Composition: `fit_ratio` (loss-ratio umbrella
with delta-method SE). All return an object carrying a `$full`
projection table.
