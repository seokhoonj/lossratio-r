...
the actual column type in `$labels[[grp]]` / `$breakpoints[[grp]]` may
be factor, character, or even Date. This converts the character name
back to a scalar of the column's type for `==` filtering.
