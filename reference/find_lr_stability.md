...
Identify the first valuation \\k\_{stable}\\ from which the projected
loss ratio is *predictively* stable, in the sense of the paper's Section
11 \\k_0\\ criterion:

\$\$k\_{stable} = \min\\v \in \[k^\*, V - h\] : R_v \< c \cdot
...
This function corresponds to \\k_0\\ in the paper. The notation "0"
denotes the starting index of the stable regime; the function name uses
`lr_stability` to be self-documenting.
...
An object of class `LRStability` (named list) containing the detected
`k_stable`, the candidate sequence `v`, and the diagnostic sequences
