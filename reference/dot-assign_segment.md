...
Returns `rep(1L, length(cohort_vals))` when `regime` is `NULL` or
...
``` r
.assign_segment(cohort_vals, regime, group_cols = NULL)
```
...
- group_cols:
  ...
  Optional `data.table` (`nrow == length(cohort_vals)`) carrying the
  group columns named in `regime$groups`. Required when `regime` is
  multi-group; ignored otherwise. Each row's segment is computed against
  the change points for that row's group.

## Value

Integer vector of segment ids, same length as `cohort_vals`.
