...
calibration scalars (target ultimate LR, cohort prem volume, cell-level
noise CV) were measured once on a real long-term Korean health-insurance
...
A `data.table` with 2,664 rows and 15 columns. Each row is one
(coverage, underwriting cohort, calendar period) cell of a 36-cohort x
up-to-36-dev jagged triangle:
...
Generated by `data-raw/make_experience.R`. Calibration scalars
(per-coverage target LR, prem volume mean + CV across cohorts,
cell-level noise CV) were measured once on a real portfolio and baked in
as constants; cell-level structure and all noise draws are synthetic.
