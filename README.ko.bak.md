...
- age-to-age (`ATA`) 와 노출 기반 (`ED`) 의 경과 기간 모형화 — Worker
  계층 (`fit_cl`, `fit_ed`, `fit_ata`, `fit_intensity`)
...
- 손해율 추정 (`fit_lr`) — loss 와 premium 적합을 합성, 세 가지 method:
  ...
  - `"cl"` — 고전적 chain ladder (Mack 모형) 표준오차는
    `se_method = "fixed"` (보험료를 known 으로 처리) 또는 `"delta"` (L/P
    에 delta method) 로 산출; 신뢰구간은 `conf_level` 인자로 제어;
    실증적 bootstrap CI 옵션도 제공.
...
`premium_incr`).
[`build_triangle()`](https://seokhoonj.github.io/lossratio/reference/build_triangle.md)
은 출력에서 두 형태를 모두 생성한다. Worker 적합 함수 (`fit_cl`,
`fit_ed`, `fit_ata`, `fit_intensity`) 는 generic 한 `target` /
`exposure` / `weight` 인자를 받고, 디스패처 (`fit_loss`, `fit_premium`)
와 합성 적합 (`fit_lr`) 은 역할별 `loss_*` / `premium_*` 인자명을
사용한다. default 는 누적 형 컬럼명 (`"loss"`, `"premium"`).
...
`uy_a` 등) 와 무관하게 `cohort` 와 `dev` 로 표준화된다. 원본 컬럼명은
attribute (`cohort_var`, `calendar_var`, `dev_var`) 로 보존되며, 집계
주기는 `grain` (`"M"` / `"Q"` / `"S"` / `"A"`) 으로 저장된다.
