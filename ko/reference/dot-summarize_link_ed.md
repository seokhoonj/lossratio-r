...
(built with `exposure_var` set). Dispatched via
...
- object:
  ...
  A `Link` object built with `exposure_var` set, typically produced by
...
- `g`:
  ...
  WLS-estimated intensity from `lm(delta_value ~ exposure_from + 0)`.
