...
Role dispatcher: `fit_loss` (loss 측 sa/ed/cl), `fit_premium` (premium
측 ed/cl). 합성: `fit_lr` (손해율 통합 인터페이스, delta-method SE).
...
parameter + process). 반환 객체는 `fit_loss` / `fit_premium` / `fit_lr`
의 `bootstrap` 인자에 전달되어 분석식 SE / CI 를 경험적 값으로 교체.
