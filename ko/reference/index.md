...
Role dispatcher: `fit_loss` (loss 측 sa/ed/cl), `fit_prem` (premium 측
ed/cl). 합성: `fit_lr` (손해율 통합 인터페이스, delta-method SE). 모두
결과 객체에 `$full` projection table 을 보유.
...
`fit_lr(method = "sa")` 의 stage transition 에서 사용되고,
...
`loss_regime` / `prem_regime` 인자에 들어갈 수동 (`_at()`) 혹은
...
parameter + process). 반환 객체는 `fit_loss` / `fit_prem` / `fit_lr` 의
`bootstrap` 인자에 전달되어 분석식 SE / CI 를 경험적 값으로 교체.
