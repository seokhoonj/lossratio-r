...
Triangle 위에서 projection 을 산출하는 모형. `fit_cl` (chain ladder),
`fit_ed` (노출 기반 / additive), `fit_lr` (손해율 통합 인터페이스 —
`"sa"` 단계 적응형 default, `"ed"`, `"cl"` method 지원). 모두 결과
...
링크별 ATA 인자 추정 (factor level). projection 은 산출하지 않고
인자·SE·진단 stat 만 제공. `fit_cl` 의 building block 이며
[`detect_maturity()`](https://seokhoonj.github.io/lossratio/ko/reference/detect_maturity.md)
와 `fit_lr(method = "sa")` 의 stage transition 에서 내부적으로 사용. ED
측 factor-level 진단은 별도 함수가 아닌 `summary.Link(model = "ed")` 로
노출.
