...
> (`fit_ratio`) 이며, 준비금 framing 은 거기서는 직접 적용되지 않는다.
> 이 글은 P&C 배경에서 오는 사용자가 `fit_cl` 을 익숙한 Mack chain
> ladder workflow 와 매핑해 볼 수 있도록 하는 참고 자료로 둔다.
>
...
cl_tail <- fit_cl(tri, loss = "loss", method = "mack", tail = TRUE)

# 또는 명시적인 tail 인자 값 지정
cl_tail <- fit_cl(tri, loss = "loss", method = "mack", tail = 1.025)
```
...
fit_cl(tri, loss = "loss", method = "mack", sigma_method = "loglinear")
#> <CLFit>
#> method      : mack 
#> loss        : loss 
#> weight      : none 
