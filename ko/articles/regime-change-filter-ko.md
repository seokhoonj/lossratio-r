...
`Regime` / `"auto"` / 클로저) 를 노출한다. 도메인 지식으로 change 날짜를
직접 지정하려면 `regime_at(change = "2024-07-01")` 로 `Regime` 객체를
만들어 전달한다
...
미관측. 수직 점선은 성숙점 k^\*, 수평 점선은 regime change 코호트 경계를
나타낸다.](figs/triangle_usage_panels.png)
