...
[`fit_ratio()`](https://seokhoonj.github.io/lossratio/ko/reference/fit_ratio.md)
is the unified loss-ratio interface. The default `method = "ed"`
(exposure-driven) is the unconditional safe baseline – no maturity or
regime detection needed. Use `method = "cl"` for cohort-anchored
projection under cohort-level drift, or `method = "sa"` (stage-adaptive)
for long-tail portfolios where both volatile early dev and cohort drift
coexist.
