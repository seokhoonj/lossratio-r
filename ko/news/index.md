...
- **BREAKING: `Regime` treatment redesign – two full-development
  ...
  - `"segment_bridged_borrowed"` estimates factors per segment
    ...
    [`.borrow_segment_factors()`](https://seokhoonj.github.io/lossratio-r/ko/reference/dot-borrow_segment_factors.md)
  ...
  *before* the `Link` is built, because the `Link` omits dev-1-only
  ...
  [`.compute_segment_mini_tri_bounds()`](https://seokhoonj.github.io/lossratio-r/ko/reference/dot-compute_segment_mini_tri_bounds.md)
  ...
  and
  [`.compute_triangle_usage()`](https://seokhoonj.github.io/lossratio-r/ko/reference/dot-compute_triangle_usage.md).
...
- The `maturity_from` column reported for `method = "sa"` fits (in
  [`summary()`](https://rdrr.io/r/base/summary.html) and the underlying
  `$full` table) now carries the maturity link’s to-index (`ata_to`, the
  first CL-phase dev) instead of its from-index, matching the package
  convention that a maturity point is a link target dev. Projections,
  standard errors, and confidence intervals are unchanged – only the
  reported column value shifts up by one.
...
- **API consistency pass.** Several entry points were aligned so the
  ...
  - `premium_method` now defaults to `"ed"` (was `"cl"`) in
    ...
    old behaviour.
  - [`bootstrap()`](https://seokhoonj.github.io/lossratio-r/ko/reference/bootstrap.md)’s
...
- **`fit_sa` worker + `fit_loss` true dispatcher.** Phase 4 split the
  ...
  [`.premiumfit_augment()`](https://seokhoonj.github.io/lossratio-r/ko/reference/dot-premiumfit_augment.md) +
  [`.premiumfit_bootstrap()`](https://seokhoonj.github.io/lossratio-r/ko/reference/dot-premiumfit_bootstrap.md)
  (Phase 4c).
...
- **Variance helper rename — `.mack_g_var` -\> `.ed_g_var`.** Internal
  ...
  lives at
  [`.ed_g_var()`](https://seokhoonj.github.io/lossratio-r/ko/reference/dot-ed_g_var.md).
  The two natural analytical variance helpers in the package are now:
  [`.mack_f_var()`](https://seokhoonj.github.io/lossratio-r/ko/reference/dot-mack_f_var.md)
  (CL paradigm, f) and
  [`.ed_g_var()`](https://seokhoonj.github.io/lossratio-r/ko/reference/dot-ed_g_var.md)
