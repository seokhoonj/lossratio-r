...
| $`k^*`$ | maturity point (from [`find_ata_maturity()`](https://seokhoonj.github.io/lossratio/reference/find_ata_maturity.md)); lower bound on candidate $`v`$ |
| $`k^{**}`$ | convergence point — the value [`find_lr_convergence()`](https://seokhoonj.github.io/lossratio/reference/find_lr_convergence.md) returns |
