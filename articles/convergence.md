# Convergence: detecting when projected loss ratio stops revising
...
| $`k^*`$ | maturity point (from [`detect_maturity()`](https://seokhoonj.github.io/lossratio/reference/detect_maturity.md)); lower bound on candidate $`v`$ |
| $`k^{**}`$ | convergence point — the value [`detect_convergence()`](https://seokhoonj.github.io/lossratio/reference/detect_convergence.md) returns |
...
library(lossratio)
data(experience)
tri <- build_triangle(experience[coverage == "SUR"], coverage)
...
- `se_mult`, `max_dv`, `min_run`, `holdout_max`, `min_n_cohorts` —
  settings used.
- attributes `group_var`, `target`, `fit_fn_name`, `dev_var`.
...
|----|----|----|----|
| [`detect_regime()`](https://seokhoonj.github.io/lossratio/reference/detect_regime.md) | Are cohorts homogeneous? | cohort groups | underwriting period |
