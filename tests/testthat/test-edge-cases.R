...
test_that("build_triangle accepts a zero-row data.frame and returns empty Triangle", {
  ...
  expect_no_error(tri <- build_triangle(exp_empty, groups = coverage))
...
test_that("build_triangle on a single cohort succeeds", {
  ...
  tri <- build_triangle(single, groups = coverage)
  expect_s3_class(tri, "Triangle")
...
test_that("build_triangle on a single group succeeds", {
  ...
  tri <- build_triangle(one_grp, groups = coverage)
  expect_s3_class(tri, "Triangle")
...
test_that("build_triangle propagates NA loss without erroring", {
  ...
  expect_no_error(tri <- build_triangle(exp_na, groups = coverage))
  expect_s3_class(tri, "Triangle")
...
test_that("build_triangle errors on grain finer than input (uy_a + grain='M')", {
  ...
  expect_error(
    build_triangle(exp, groups = coverage,
                   cohort = "uy_a", calendar = "cy_a", grain = "M"),
    regexp = "grain"
  )
})
...
test_that("build_calendar with calendar_var = 'cy_q' returns Calendar quarter", {
  ...
  cal_q <- build_calendar(exp, groups = coverage, calendar = "cy_q")
