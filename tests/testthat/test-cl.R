...
test_that("$full has expected columns", {
  cl <- fit_cl(tri, loss = "loss", method = "mack")
  for (nm in c("coverage", "cohort", "dev", "loss_obs", "loss_proj", "is_observed")) {
    expect_true(nm %in% names(cl$full), info = paste("missing", nm))
  }
})

test_that("loss_proj is finite for projected cells", {
  cl <- fit_cl(tri, loss = "loss", method = "mack")
  proj_only <- cl$full[is_observed == FALSE]
  expect_true(all(is.finite(proj_only$loss_proj)))
})

test_that("fit_cl method = 'mack' adds variance columns", {
  cl <- fit_cl(tri, loss = "loss", method = "mack")
  for (nm in c("loss_proc_se", "loss_param_se",
...
test_that("Mack standard errors are non-negative", {
  cl <- fit_cl(tri, loss = "loss", method = "mack")
  ses <- cl$full[is.finite(loss_total_se), loss_total_se]
